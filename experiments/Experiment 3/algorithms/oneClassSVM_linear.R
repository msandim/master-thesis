...
one_class_svm_linear_train <- function(data)
{
  ...
  model <- svm(data_X, kernel = "linear", type='one-classification', scale = FALSE)
  
  return(list(model = model, scaleObject = scaleObject))
}

one_class_svm_linear_test <- function(model, data)
{
  data_X <- data %>% select(-outlier)
  ...
  returnData$oneClassSVM_linear <- ifelse(predictions, "no", "yes")
