...
one_class_svm_radial_train <- function(data)
{
  ...
  model <- svm(data_X, kernel = "radial", type='one-classification', scale = FALSE)
  
  return(list(model = model, scaleObject = scaleObject))
}

one_class_svm_radial_test <- function(model, data)
{
  data_X <- data %>% select(-outlier)
  ...
  returnData$oneClassSVM_radial <- ifelse(predictions, "no", "yes")
