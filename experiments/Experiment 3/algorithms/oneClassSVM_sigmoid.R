...
one_class_svm_sigmoid_train <- function(data)
{
  ...
  model <- svm(data_X, kernel = "sigmoid", type='one-classification', scale = FALSE)
  
  return(list(model = model, scaleObject = scaleObject))
}

one_class_svm_sigmoid_test <- function(model, data)
{
  data_X <- data %>% select(-outlier)
  ...
  returnData$oneClassSVM_sigmoid <- ifelse(predictions, "no", "yes")
